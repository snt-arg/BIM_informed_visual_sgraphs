...
namespace ORB_SLAM3
{
                                                                                         ...
                                                                                         mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
    {
        ...
        // Add a-graph data structure to the Atlas
    ...
    void System::UpdateWallsFromMsg(const vs_graphs::msg::BIMWallDataArray &BimWalls) {
    //log info of updating walls from markers
      cout << "Updating walls from markers..." << endl;
      // print size og BimWalls
      cout << "Number of walls in BimWalls: " << BimWalls.walls.size() << endl;
      for (const auto& wall : BimWalls.walls) {
        Plane* newPlane = new Plane();
        newPlane->setPlaneType(Plane::BIM_WALL);
        newPlane->setBIMId(wall.id);
        newPlane->setStartX(wall.start.x);
        newPlane->setStartY(wall.start.y);
        newPlane->setStartZ(wall.start.z);
        newPlane->setLength(wall.length);
        newPlane->setCentroid(Eigen::Vector3f(wall.centroid.x, wall.centroid.y, wall.centroid.z));

        // print id and wall.distance
        // cout << "Wall ID: " << wall.id << ", Wall distance: " << wall.distance << endl;
        Eigen::Vector4d coeffs;
        coeffs << wall.normal.x, wall.normal.y, wall.normal.z, wall.distance;
        g2o::Plane3D globalEquation(coeffs);
        // newPlane->setLength(wall.length);
        newPlane->setGlobalEquation(globalEquation);

        // cout << std::fixed << std::setprecision(6) 
        //         << "New Plane created: "
        //         << "Type: " << newPlane->getPlaneType()
        //         << ", ID: " << newPlane->getBIMId()
        //         << ", StartX: " << newPlane->getStartX()
        //         << ", StartY: " << newPlane->getStartY()
        //         << ", StartZ: " << newPlane->getStartZ()
        //         << ", Length: " << newPlane->getLength()
        //         << ", Normal: (" << newPlane->getGlobalEquation().normal().transpose()
        //         << ", Distance: " << newPlane->getGlobalEquation().distance()
        //         << ")";

        mpAtlas->GetBIMDatabase().AddWallBIM(newPlane);
        }
        
    }


    void System::UpdateWallsFromMarkers(const visualization_msgs::msg::MarkerArray &BimMarkers) {
    //log info of updating walls from markers
    cout << "Updating walls from markers..." << endl;
        for (const auto& marker : BimMarkers.markers)
        {
            if (marker.type == visualization_msgs::msg::Marker::CUBE)
            {
                // From BIM frame to World frame
                int wall_id = marker.id;
                float x_min = -marker.pose.position.y;
                float y_min = 0.0;
                float z_min = marker.pose.position.x; 
                float length = marker.pose.position.z;
                float n_x = -marker.pose.orientation.y;
                float n_y = 0.0;
                float n_z = marker.pose.orientation.x; // Should be 0.0
                double plane_dist = marker.pose.orientation.w;

                // Construct the plane equation: ax + by + cz + d = 0
                // Here, a = n_x, b = n_y, c = n_z, d = plane_dist
                Eigen::Vector4d coeffs;
                coeffs << n_x, n_y, n_z, plane_dist;
                g2o::Plane3D globalEquation(coeffs);

                // Create and fill Plane object
                Plane* newPlane = new Plane();
                newPlane->setPlaneType(Plane::BIM_WALL);
                newPlane->setBIMId(wall_id);
                newPlane->setStartX(x_min);
                newPlane->setStartY(y_min);
                newPlane->setStartZ(z_min);
                // if (plane_dist <= 0.0001)
                // {
                ...
                // Optionally set centroid, thickness...
                // newPlane->setCentroid(Eigen::Vector3f(x_min + length / 2, y_min + length / 2, 0.0));
                // newPlane->setColor({255, 0, 0}); // Red color

                // Print the newPlane information
                cout << std::fixed << std::setprecision(6) 
                 << "New Plane created: "
                    << "Type: " << newPlane->getPlaneType()
                    << ", ID: " << newPlane->getBIMId()
                ...
                // Add the Plane to the Atlas BimWalls Database
                // mpAtlas->GetCurrentMap()->AddMapPlane(newPlane);
                // mpAtlas->AddBIMWall(newPlane);
                mpAtlas->GetBIMDatabase().AddWallBIM(newPlane);
                // auto walls = mpAtlas->GetBIMDatabase().GetWallsBIM();
            }

            //  Optimizer::GlobalBundleAdjustemnt(pActiveMap, 10, &mbStopGBA, nLoopKF, false,
            //                                       mpTracker->GetMarkerImpact());
        }
        int nIterations = 10; // or whatever number of iterations you want
            bool stopFlag = false;
            unsigned long nLoopKF = 0; // or the appropriate keyframe id
            bool bRobust = true; // or false, depending on your needs
            double markerImpact = 1.0; // or whatever value is appropriate
    ...
    // vector<Plane *> System::GetAllBIMWalls()
    // {
    //     // Map *pActiveMap = mpAtlas->GetCurrentMap();
    //     // return pActiveMap->GetAllBIMWalls();
    // }
