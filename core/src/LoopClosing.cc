...
namespace ORB_SLAM3
{
    ...
    void LoopClosing::MergeLocal()
    {
        ...
        // vector<Plane *> vpCurrentMapBIMPlanes = pCurrentMap->GetAllBIMWalls();
        ...
        if (vpCurrentMapKFs.size() != 0)
        {
            ...
            // When all the elements are updated, move them to the new map
            {
                ...
                // Loop over the BIM Planes of the current map and move them to the new map
                // for (Plane *pPlane : vpCurrentMapBIMPlanes)
                // {
                //     if (!pPlane)
                //         continue;

                //     pPlane->SetMap(pMergeMap);
                //     pPlane->setId(pMergeMap->GetAllBIMWalls().size());
                //     pMergeMap->AddMapPlane(pPlane);
                //     pCurrentMap->EraseMapPlane(pPlane);
                // }
    ...
    void LoopClosing::RunGlobalBundleAdjustment(Map *pActiveMap, unsigned long nLoopKF)
    {
        ...
        if (!bImuInit)
            ...
            std::cout << "FullInertialBA LoopCLosing" << std::endl;
