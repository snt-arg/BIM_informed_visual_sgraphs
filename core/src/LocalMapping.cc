...
namespace ORB_SLAM3
{
            ...
            if (CheckNewKeyFrames() && !mbBadImu)
            {
                ...
                if (!CheckNewKeyFrames() && !stopRequested())
                {
                    if (mpAtlas->KeyFramesInMap() > 2)
                    {
                        ...
                        {
                            // get the frecuency of calling the LocalBundleAdjustment
                            // std::cout << " calling LOCAL BA"<<std::endl; 
                            Optimizer::LocalBundleAdjustment(mpAtlas, mpCurrentKeyFrame, &mbAbortBA, mpCurrentKeyFrame->GetMap(),
                        ...
                        if (runWithAgraph && timeSinceLastBIM >= 5 && HasSufficientBIMData())
                        {
                                ...
                                !mpAtlas->GetBIMDatabase().GetWallsBIM().empty())
                            {
                                try {
                                    ...
                                    lastBIMOptTime = currentTime;
                                    std::cout << "BIM optimization completed successfully!" << std::endl;
                                } catch (const std::exception& e) {
                                    std::cout << "BIM optimization failed: " << e.what() << std::endl;
                                }
                            }
                        }
                    }
    ...
    bool LocalMapping::HasSufficientBIMData()
    {
        return !mpAtlas->GetBIMDatabase().GetWallsBIM().empty() && 
            mpAtlas->GetBIMDatabase().GetWallsBIM().size() >= 1; // Adjust threshold as needed
    }
