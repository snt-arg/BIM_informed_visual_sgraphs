...
std::string frameWorld, frameCamera, frameImu, frameMap, frameBC, frameSE, frameBIM;
...
// BIM related
rclcpp::Publisher<visualization_msgs::msg::MarkerArray>::SharedPtr pubBIMWalls;
rclcpp::Publisher<visualization_msgs::msg::MarkerArray>::SharedPtr optimizedWalls;
rclcpp::Publisher<visualization_msgs::msg::MarkerArray>::SharedPtr pubBIMOptimizedTrajectory;
rclcpp::Publisher<visualization_msgs::msg::MarkerArray>::SharedPtr pubBIMBEFOREOptimizedTrajectory;

// Future BIM publishers:
...
void setupPublishers(std::shared_ptr<rclcpp::Node> node, std::shared_ptr<image_transport::ImageTransport> image_transport, const std::string &node_name)
{
    ...
    // BIM Elements
    pubBIMWalls = node->create_publisher<visualization_msgs::msg::MarkerArray>(node_name + "/bim_walls", 1);
    optimizedWalls = node->create_publisher<visualization_msgs::msg::MarkerArray>(node_name + "/optimized_walls", 1);
    pubBIMOptimizedTrajectory = node->create_publisher<visualization_msgs::msg::MarkerArray>(node_name + "/bim_optimized_trajectory", 1);
    pubBIMBEFOREOptimizedTrajectory = node->create_publisher<visualization_msgs::msg::MarkerArray>(node_name + "/bim_before_optimized_trajectory", 1);
    // pubLocalBAGraph = node->create_publisher<visualization_msgs::msg::MarkerArray>(
    // node_name + "/local_ba_graph", 1);
    ...
    visualTools->loadMarkerPub(false);
...
void publishTopics(rclcpp::Time msgTime, Eigen::Vector3f Wbb)
{
    ...
    publishAllMappedWalls(pSLAM->GetAllPlanes(), msgTime);         // For GNN
    // BIM
    std::vector<ORB_SLAM3::Plane*> bimWalls = pSLAM->GetAtlas()->GetBIMDatabase().GetWallsBIM();
    publishBIMWalls(bimWalls, msgTime);
    // publishOptimizedWalls(pSLAM->GetAllPlanes(), msgTime);
    // publishBIMOptimizedKeyFrames(msgTime);
    // publishBIMBEFOREOptimizedKeyFrames(msgTime);

    // Publish pointclouds
    if (pubPointClouds)
    {
        publishSegmentedCloud(keyframes, msgTime);           // the ones below
        publishPlanes(pSLAM->GetAllPlanes(), msgTime);       // the point clouds on top
...
void publishBIMBEFOREOptimizedKeyFrames(rclcpp::Time msgTime)
{
    // Get the optimized KeyFrames from the global storage - CALL AS STANDALONE FUNCTION
    std::vector<ORB_SLAM3::KeyFrame*> optimizedKFs = ORB_SLAM3::GetBEFOREOptimizedKeyFrames();\n    \n    if (optimizedKFs.empty()) {
        return; // No optimized KeyFrames available yet
    }

    visualization_msgs::msg::MarkerArray optimizedTrajectory;
    
    // Create trajectory line for BIM-optimized poses
    visualization_msgs::msg::Marker trajLine;
    trajLine.header.frame_id = frameBIM;
    trajLine.header.stamp = msgTime;
    trajLine.id = 0;
    trajLine.type = visualization_msgs::msg::Marker::LINE_STRIP;
    trajLine.action = visualization_msgs::msg::Marker::ADD;
    trajLine.lifetime = rclcpp::Duration::from_seconds(0);
    trajLine.ns = "bim_BEFOREoptimized_keyframes";
    
    trajLine.scale.x = 0.03;  // Thicker line to highlight BIM optimization
    trajLine.color.r = 1.0;   // Bright colors to distinguish from regular trajectory
    trajLine.color.g = 0.0;
    trajLine.color.b = 1.0;   // Magenta
    trajLine.color.a = 0.8;
    
    // Create keyframe markers for BIM-optimized poses
    visualization_msgs::msg::Marker kfMarkers;
    kfMarkers.header.frame_id = frameBIM;
    kfMarkers.header.stamp = msgTime;
    kfMarkers.id = 1;
    kfMarkers.type = visualization_msgs::msg::Marker::SPHERE_LIST;
    kfMarkers.action = visualization_msgs::msg::Marker::ADD;
    kfMarkers.lifetime = rclcpp::Duration::from_seconds(0);
    kfMarkers.ns = "bim_BEFOREoptimized_keyframes";
    
    kfMarkers.scale.x = 0.08;  // Larger spheres
    kfMarkers.scale.y = 0.08;
    kfMarkers.scale.z = 0.08;
    kfMarkers.color.r = 0.0;
    kfMarkers.color.g = 1.0;
    kfMarkers.color.b = 1.0;   // Orange
    kfMarkers.color.a = 0.9;
    
    // Add poses to visualization
    for (const auto& kf : optimizedKFs) {
        if (kf->isBad()) continue;
        
        // Get the BIM-optimized pose
        Sophus::SE3f optimizedPose = kf->GetPose().inverse(); // World-to-camera inverse = camera pose in world
        
        geometry_msgs::msg::Point point;
        point.x = optimizedPose.translation().x();
        point.y = optimizedPose.translation().y();
        point.z = optimizedPose.translation().z();
        
        trajLine.points.push_back(point);
        kfMarkers.points.push_back(point);
    }
    
    
    optimizedTrajectory.markers.push_back(trajLine);
    optimizedTrajectory.markers.push_back(kfMarkers);
    
    // Publish (you'll need to add this publisher in setupPublishers)
    if (pubBIMBEFOREOptimizedTrajectory) {
        pubBIMBEFOREOptimizedTrajectory->publish(optimizedTrajectory);
    }
}

void publishBIMOptimizedKeyFrames(rclcpp::Time msgTime)
{
    // Get the optimized KeyFrames from the global storage - CALL AS STANDALONE FUNCTION
    std::vector<ORB_SLAM3::KeyFrame*> optimizedKFs = ORB_SLAM3::GetOptimizedKeyFrames();
    ...
    trajLine.action = visualization_msgs::msg::Marker::ADD;
    trajLine.lifetime = rclcpp::Duration::from_seconds(0);
    trajLine.ns = "bim_optimized_trajectory";
    ...
    kfMarkers.action = visualization_msgs::msg::Marker::ADD;
    kfMarkers.lifetime = rclcpp::Duration::from_seconds(0);
    kfMarkers.ns = "bim_optimized_keyframes";
    
    kfMarkers.scale.x = 0.08;  // Larger spheres
    kfMarkers.scale.y = 0.08;
    kfMarkers.scale.z = 0.08;
    kfMarkers.color.r = 1.0;
    kfMarkers.color.g = 0.5;
    kfMarkers.color.b = 0.0;   // Orange
    kfMarkers.color.a = 0.9;
    ...
    // Publish (you'll need to add this publisher in setupPublishers)
    if (pubBIMOptimizedTrajectory) {
        pubBIMOptimizedTrajectory->publish(optimizedTrajectory);
    }
}

void publishOptimizedWalls(const std::vector<ORB_SLAM3::Plane*>& allPlanesVec, rclcpp::Time msgTime)
{
    static rclcpp::Time lastPublishTime(0, 0, RCL_ROS_TIME);
    
    // Only publish every 2 seconds
    if ((msgTime - lastPublishTime).seconds() < 1.0) {
        return;
    }
    lastPublishTime = msgTime;
    ...
    deleteAllMarker.ns = "optimized_walls";
    deleteAllMarker.action = visualization_msgs::msg::Marker::DELETEALL;
    clearArray.markers.push_back(deleteAllMarker);
    optimizedWalls->publish(clearArray);

    if (allPlanesVec.empty())
        return;

    visualization_msgs::msg::MarkerArray wallArray;
    int marker_id = 0;

    for (size_t idx = 0; idx < allPlanesVec.size(); idx++)
    {
        const auto& plane = allPlanesVec[idx];
        // if (!plane || plane->getPlaneType() != ORB_SLAM3::Plane::planeVariant::WALL)
        //     continue;

        // Get the global plane equation
        g2o::Plane3D globalEquation = plane->getGlobalEquation();
        Eigen::Vector3d normal = globalEquation.normal();
        // double distance = globalEquation.distance();

        Eigen::Vector4d coeffs = globalEquation.coeffs(); // [nx, ny, nz, d]
        double distance = coeffs(3);
                
        // Get plane centroid for positioning
        // Eigen::Vector3f centroid = plane->getCentroid();
        Eigen::Vector3d centroid = -distance * normal.normalized();
        
        // Create wall marker
        visualization_msgs::msg::Marker wallMarker;
        wallMarker.header.frame_id = frameBIM;
        wallMarker.header.stamp = msgTime;
        wallMarker.id = marker_id++;
        wallMarker.type = visualization_msgs::msg::Marker::CUBE;
        wallMarker.action = visualization_msgs::msg::Marker::ADD;
        wallMarker.lifetime = rclcpp::Duration::from_seconds(1);
        wallMarker.ns = "optimized_walls";

        // Position at centroid
        wallMarker.pose.position.x = centroid.x();
        wallMarker.pose.position.y = centroid.y();
        wallMarker.pose.position.z = centroid.z();

        // Calculate orientation from normal vector
        Eigen::Vector3d defaultNormal(0, 0, 1);
        Eigen::Quaterniond rotation = Eigen::Quaterniond::FromTwoVectors(defaultNormal, normal);
        
        wallMarker.pose.orientation.x = rotation.x();
        wallMarker.pose.orientation.y = rotation.y();
        wallMarker.pose.orientation.z = rotation.z();
        wallMarker.pose.orientation.w = rotation.w();

        // Simple fixed size
        wallMarker.scale.x = 4.0; // width
        wallMarker.scale.y = 2.0; // height  
        wallMarker.scale.z = 0.1; // thickness

        // get id of the detected wall
        int bim_id = plane->getBIMId();

        if (bim_id == 1 || bim_id == 3)
        {
            RCLCPP_DEBUG(rclcpp::get_logger("vs_graphs"), "Plane matched with BIM %d", bim_id);

            // Simple color - green for optimized walls
            wallMarker.color.r = 0.0;
            wallMarker.color.g = 1.0;
            wallMarker.color.b = 0.0;
            wallMarker.color.a = 0.6;
        }else{

            // // Simple color - red for non-optimized walls
            // wallMarker.color.r = 1.0;
            // wallMarker.color.g = 0.0;
            // wallMarker.color.b = 0.0;
            // wallMarker.color.a = 0.6;
            // dont push the wall marker if it is not matched with BIM
            continue;
        }
        
        wallArray.markers.push_back(wallMarker); 
    }

    // Publish the optimized walls
    if (!wallArray.markers.empty()) {
        optimizedWalls->publish(wallArray);
    }
}
...
void publishBIMWalls(const std::vector<ORB_SLAM3::Plane*>& bimWalls, rclcpp::Time msgTime)
{
    if (bimWalls.empty())
        return;

    visualization_msgs::msg::MarkerArray bimWallArray;
    int marker_id = 0;
    ...
    deleteAllMarker.header.frame_id = frameBIM.empty() ? frameWorld : frameBIM;
    deleteAllMarker.header.stamp = msgTime;
    deleteAllMarker.ns = "bim_walls"; // Must match the namespace below
    deleteAllMarker.action = visualization_msgs::msg::Marker::DELETEALL;
    ...
    deleteXWalls.ns = "bim_x_walls";
    deleteXWalls.action = visualization_msgs::msg::Marker::DELETEALL;
    deleteXWalls.id = 0;
    bimWallArray.markers.push_back(deleteXWalls);

    visualization_msgs::msg::Marker deleteYWalls;
    deleteYWalls.header.frame_id = frameBIM.empty() ? frameWorld : frameBIM;
    deleteYWalls.header.stamp = msgTime;
    deleteYWalls.ns = "bim_y_walls";
    deleteYWalls.action = visualization_msgs::msg::Marker::DELETEALL;
    deleteYWalls.id = 0;
    bimWallArray.markers.push_back(deleteYWalls);

    for (size_t idx = 0; idx < bimWalls.size(); idx++)
    {
        const auto& wall = bimWalls[idx];
        if (!wall)
            continue;
        ...
        wallMarker.type = visualization_msgs::msg::Marker::CUBE;
        wallMarker.action = visualization_msgs::msg::Marker::ADD;
        wallMarker.lifetime = rclcpp::Duration::from_seconds(0);
            
        // Position: start point + half length in Y direction
        auto centroid = wall->getCentroid();
    ...
    // Publish the BIM walls
    pubBIMWalls->publish(bimWallArray);
}
...
void publishAllMappedWalls(std::vector<ORB_SLAM3::Plane *> walls, rclcpp::Time msgTime)
{
    ...
    for (const auto &wall : walls)
    {
        ...
        if (wallCloud && wallCloud->points.size() > 1)
        {
            ...
            Eigen::Vector3f geometricCenter;
            geometricCenter.x() = (min_pt.x + max_pt.x) / 2.0f;
            ...
            float extent_x = max_pt.x - min_pt.x;
            float extent_y = max_pt.y - min_pt.y; 
            float extent_z = max_pt.z - min_pt.z;
        ...
        // float extent_x = max_pt.x - min_pt.x; // width along X (choose what “width” means)
        // float extent_y = max_pt.y - min_pt.y; // height along Y
        // float extent_z = max_pt.z - min_pt.z; // thickness along Z
...
void publishKeyFrameMarkers(std::vector<ORB_SLAM3::KeyFrame *> keyframe_vec, rclcpp::Time msgTime)
{
    ...
    for (auto &keyframe : keyframe_vec)
    {
        ...
        auto ts = keyframe->mTimeStamp;

        pose.header.stamp = rclcpp::Time(ts * 1e9); // Convert seconds to nanoseconds
