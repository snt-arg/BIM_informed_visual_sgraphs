...
public:
    ...
    // void GrabBimWalls(const visualization_msgs::msg::MarkerArray &msgBimWalls);
...
int main(int argc, char **argv)
{
    ...
    rclcpp::Subscription<vs_graphs::msg::BIMWallDataArray>::SharedPtr bin_wall_data_sub_;
    ...
    [&subCsvData](const vs_graphs::msg::BIMWallDataArray& msg) {
        ...
        RCLCPP_INFO(rclcpp::get_logger("ImageGrabber"), "Processing BIM data from CSV (one-time)");
        if (pSLAM) {
            pSLAM->UpdateWallsFromMsg(msg);
            bimDataProcessed = true; // Mark as processed
            subCsvData.reset(); // Now it's safe to reset
        } else {
            RCLCPP_ERROR(rclcpp::get_logger("ImageGrabber"), "[Error] pSLAM is not initialized. Cannot update walls from markers.");
        }
    });
...
//  * @brief Callback function to get the skeleton graph from the `voxblox` module
//  *
//  * @param msgBimWalls The skeleton graph from the `voxblox` module
//  */
// void ImageGrabber::GrabBimWalls(const visualization_msgs::msg::MarkerArray &msgBimWalls)
// {
//     // Pass the skeleton graph to a buffer to be processed by the SemanticSegmentation thread
//     setBimWalls(msgBimWalls);
// }
